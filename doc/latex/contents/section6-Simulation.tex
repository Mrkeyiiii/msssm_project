...
\begin{figure}[ht]
	...
	\caption{Simulation of the evacuation procedure of $N=600$ agents, through two $\SI{4.7}{\meter}$ wide exits, with a panic value of $p=0.4$, at four different time instants. The whole evacuation procedure takes $\SI{21}{\second}$. (a): The agents choose an exit and start moving. (b): The agents have divided into two groups. (c): The first agents have reached the exits. (d): Arch-like blocking of the exists has occurred. (e): Slow evacuation due to congestion around exits. (f): Almost all the agents have been evacuated.}
