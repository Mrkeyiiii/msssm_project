...
The model that we used to capture the crowd dynamics is the \emph{social force model}, which is an agent-based continuous-time model. Initially proposed in \cite{Helbing2000}, and then later augmented in \cite{Zainuddin2010} and \cite{Wang2016}. Our model was inspired by all of the above references, borrowing ideas from all of them and with the appropriate alterations wherever needed.
...
\begin{align} 
	...
	\bm{f}_{ij}^{\mathrm{psych}}(t) & = A_i \exp\left(\frac{r_{ij}-d_{ij}(t)}{B_i}\right) \bm{n}_{ij}(t) \cos^*(\varphi_{ij}(t)) \,, \label{eq:fij_psych} \\
...
\begin{align}
	...
	\bm{f}_{iw}^{\mathrm{psych}}(t) & = A_i \exp\left(\frac{r_{i}-d_{iw}(t)}{B_i}\right) \bm{n}_{iw}(t) \cos^*(\varphi_{iw}(t)) \,, \label{eq:fiw_psych} \\
...
\begin{equation} \label{eq:fik}
	\bm{f}_{ik}(t) = C_i \exp\left(\frac{r_{i}-d_{ik}(t)}{D_i}\right) \bm{n}_{ik}(t) \cos^*(\varphi_{ik}(t)) \,,
\end{equation}
